...
> 每次進入前置等待時，`PENDING_POLLS` 必為 0（CI pass 路徑已重置，或為首次初始化值）。
...
| 有 `failure` / `error` | 閱讀 CI 錯誤日誌，分析根本原因，修正後 `commit + push`；記錄 `LAST_PUSH_TIME`，重置 `PENDING_POLLS=0`，重新執行 `gh pr checks`（等待新 CI 觸發） |
