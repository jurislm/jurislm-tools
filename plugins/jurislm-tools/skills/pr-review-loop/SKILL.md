...
**一輪的定義**：CI 通過 + 讀取 Bot feedback + 修正（若有）= 一輪（`ROUND_COUNT += 1`）。

---

### 步驟一：Monitor CI

使用 Monitor tool 執行以下命令，**即時串流 CI 狀態**：
...
| 無任何 check 項目（`gh pr checks` 回傳空） | 若本輪有 push，等待 30 秒讓 Bot 完成 re-review；直接進入步驟二 |
| Monitor 本身失敗（gh CLI 錯誤、網路問題） | 等待 30 秒後重試一次；仍失敗則停止並通知使用者 |

> **注意**：push 後需等待約 15 秒，GitHub 才會為新 commit 建立 checks，再呼叫 Monitor。

**CI failure 修正**：
1. 從 Monitor 輸出讀取失敗的 check 名稱
2. 執行 `gh run view <run-id> --log-failed --repo <REPO>` 取得詳細日誌
3. 分析根本原因，修正後 commit + push；更新 `LAST_PUSH_TIME`
4. 等待 15 秒後重新執行步驟一（Monitor CI）

---

### 步驟二：等待並讀取 Bot Feedback

CI 通過後，等待 **30 秒**讓 Bot 完成 review（CodeRabbit、GitHub Copilot 等可能有延遲）。
...
| `CHANGES_REQUESTED` 或有需修正的 Bot feedback | 根據 feedback 判斷是否修正（見下方）；修正後 commit + push，更新 `LAST_PUSH_TIME`；`ROUND_COUNT += 1`；若 `ROUND_COUNT < MAX_ROUNDS` → 回到步驟一；若 `ROUND_COUNT >= MAX_ROUNDS` → **停止，執行「超過輪次停止」** |
