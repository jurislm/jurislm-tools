...
| CI 狀態 | 做法 |
|---------|------|
| 仍有 `pending` / `in_progress` | 等待 `time` 分鐘，重新執行 `gh pr checks`，直到進入最終狀態 |
| 有 `failure` / `error` | **本輪正式開始**（記錄 `ROUND_START`），直接進入 Step 2 處理錯誤 |
| 全部 `pass` / `success` | **本輪正式開始**（記錄 `ROUND_START`），進入 Step 1 |
...
> CI 未完成（pending）期間，不計入 `ROUND_COUNT`，也不執行任何步驟。
