...
**一輪的定義**：CI 處於最終狀態（pass 或 fail，非 pending）後，完整執行 Step 1 ~ Step 6，才算完成一輪（`ROUND_COUNT += 1`）。
...
每輪開始前，先用 `gh pr checks` 確認 CI 狀態：
...
| 有 `failure` / `error` | **本輪正式開始**（記錄 `ROUND_START`，重置 `PENDING_POLLS=0`），進入 **Step 1** |
| 全部 `pass` / `success` | **本輪正式開始**（記錄 `ROUND_START`，重置 `PENDING_POLLS=0`），進入 Step 1 |
| 無任何 check 項目（無 CI 設定） | **本輪正式開始**（記錄 `ROUND_START`，重置 `PENDING_POLLS=0`），進入 Step 1 |

```bash
ROUND_START=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
```

> CI 未完成（pending）期間，不計入 `ROUND_COUNT`，也不執行任何步驟。CI failure 時仍須先經過 Step 1（衝突檢查），避免在衝突未解決的基礎上修 CI 錯誤。
...
> 此步驟的 CI 狀態由「前置等待」確保已進入最終狀態，不會出現 pending。

| CI 結果 | 做法 |
|---------|------|
| 全部 pass | 繼續 Step 3 |
| 有 failure | 閱讀錯誤，分析並修正，commit + push；本輪結束（`ROUND_COUNT += 1`），回到「前置等待」開始下一輪 |
