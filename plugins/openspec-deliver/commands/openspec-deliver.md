...
**執行前先確認目標 repo**：本指令內容（GitHub Flow、feature 直接對 main
開 PR）僅適用於 `jurislm/entire`。此指令託管在 `jurislm-tools`（該 repo
本身走 develop→main 兩段式，與本指令描述的流程不同），只是存放位置，不是
作用對象。動手前先 `git remote -v` 確認目前所在 repo 是 `jurislm/entire`；
不是的話停下告知使用者，不要對錯的 repo 套用本流程。
...
2. 建立／沿用 OpenSpec 提案
   ...
   - proposal.md 含 `Closes #<issue-num>`；撰寫前完成環境盤點（維度
     清單見全域 `~/.claude/CLAUDE.md`「標準交付流程」步驟 2.5），寫進
...
5. 全部 phase 完成、經 verification-before-completion 確認有證據 →
   ...
   `Closes #<issue-num>`）→ 記下 PR number，依「PR 必做」補 labels
   （`gh api repos/jurislm/entire/issues/<pr-num>/labels`）與 assignee
   （`-f "assignees[]=terry90918"`）
