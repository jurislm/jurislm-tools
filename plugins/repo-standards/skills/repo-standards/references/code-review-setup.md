...
> 自動 Claude PR 審查（`.github/workflows/claude-code-review.yml` / `claude.yml` / Drone `claude-review` pipeline）已從 repo-standards 移除。本檔只保留 **GitHub Copilot 自訂指示**模板；review 的唯一操作契約見目標 repo `CLAUDE.md`。缺少該段時採用 `review-orchestration-template.md`，設定摘要見 SKILL.md「Code Review 設定」。
