...
description: >
  ...
  ESLint, Skill-driven code review, Linear-based change tracking, and AGENTS.md → CLAUDE.md handoff.
...
Follow the `repo-standards` skill: identify repo type → standardize any existing `AGENTS.md` to read `CLAUDE.md` → track the change in a Linear issue as the only tracking record （Spectra only when the user asks for it） → apply standard `.drone.yml` (lint/typecheck/test + release-please + Coolify deploy gating) + release-please-config.json → apply standard ESLint config → install the packaged review contract in target `CLAUDE.md` and configure its prerequisites → verify checklist.
