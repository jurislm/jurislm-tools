...
- `Bash(gh:*)`：只允許 `gh` 開頭的命令（安全限制），用於 `gh pr comment --body-file review.md`
...
本方案改用 `--body-file review.md`，由 Claude 先寫檔再用 file 發留言，完全避開 `\n#` 問題。
...
`jurislm PR #195 Claude Code Review 除錯`（2026-03-12 驗證通過）
