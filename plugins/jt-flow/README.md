...
Review fallback 順序固定為：GitHub Copilot 額度用完即略過；CodeRabbit GitHub
App／PR bot 額度或 rate limit 用完即轉 CodeRabbit CLI；CLI 額度或 rate limit
用完即轉 `superpowers:requesting-code-review`。任一層明確受限後不得繼續等待，
任一層產出真實 review 後則停止 fallback。
