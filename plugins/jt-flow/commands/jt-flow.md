...
1. 確認主目錄／repository root 目前在 `main`（`git worktree list` +
   ...
   狀態下繼續本流程
2. 確認實際 remote 名稱與目標（`git remote -v`，不可假設一定叫
   `origin`）；同時從該 remote URL 解析出明確的 `<owner>/<repo>`，
   後續所有 `git fetch`／`git push`／worktree base／`gh repo`／
   `gh issue`／`gh pr` 一律明寫這個 `<owner>/<repo>`，不依賴 `gh` 指令
   預設 repository（避免多 remote／多預設 config 時操作到錯的 repo）
3. 確認目標 repo 有對應的 GitHub repo（`gh repo view <owner>/<repo>`）；
   沒有 → **先停下**，向使用者確認要建立的 owner／repo 名稱與可見度
   （public／private），取得明確同意後才 `gh repo create <owner>/
   <repo> ...`；建立後重新 `git remote -v` 確認 fetch/push URL 指向
...
5. 全部 phase 完成、經 verification-before-completion 確認有證據 →
   ...
   <pr-num> -X PATCH -f "assignees[]=<user>"`；兩者呼叫後用 `gh pr view
   <pr-num> --repo <org>/<repo> --json labels,assignees` 驗證回傳結果
