...
1. 確認主目錄／repository root 目前在 `main`（`git worktree list` +
   ...
   狀態下繼續本流程
2. 確認實際 remote 名稱（`git remote -v`，不可假設一定叫 `origin`）；
   記下對應 GitHub 的 remote 名稱，後續所有 `git fetch`／`git push`／
   worktree base 一律用這個名稱，不寫死 `origin`
3. 確認目標 repo 有對應的 GitHub repo（`gh repo view`）；沒有 →
   **先停下**，向使用者確認要建立的 owner／repo 名稱與可見度
   （public／private），取得明確同意後才 `gh repo create`；建立後
   重新 `git remote -v` 確認 fetch/push URL 指向新 repo，並確認目前
   帳號對該 remote 有推送權限，再繼續後續步驟
