...
origin/main 建立，避免本地 main 落後漏掉已合併變更（佇列每處理完一個
`<change-name>` 就會 merge 一次，本地 main 很快就會落後）；worktree／
分支／提案名稱三者一致。
...
PR number，補 labels 與 assignee（`gh api repos/<org>/<repo>/issues/
<pr-num>/labels` / `-f "assignees[]=<user>"`）：
