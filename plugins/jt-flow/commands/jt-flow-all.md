...
2. 確認實際 remote 名稱與目標：`git remote -v` 找出對應 GitHub 的
   `<remote>` 名稱（不可假設一定叫 `origin`），`git fetch`／`git
   push`／worktree base 一律用這個 `<remote>`；同時從該 remote URL
   解析出明確的 `<owner>/<repo>`，所有 `gh repo`／`gh issue`／`gh pr`
   指令一律加 `--repo <owner>/<repo>`（或等效明寫），不依賴 `gh` 指令
