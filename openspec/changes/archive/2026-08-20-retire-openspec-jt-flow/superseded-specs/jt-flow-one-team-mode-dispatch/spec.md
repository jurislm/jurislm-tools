...
available when `jt-flow-one` is not a nested, delegated run, availability
implies `jt-flow-one` already is the top-level session, where it already has
direct `Workflow` access and is already the thing a caller can address
directly — there is nothing to wrap. The detection logic is retained as
groundwork for a future dispatch point that would not need `Workflow`
access at all.
