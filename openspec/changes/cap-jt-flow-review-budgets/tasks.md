...
- [ ] 1.1 In `openspec/specs/jt-flow-review-orchestration/spec.md`, rewrite
...
- [ ] 1.2 In the same file, rewrite the "External reviewers have one
...
- [ ] 2.1 Replace the local-review batch paragraph (the one starting "每批
      程式碼變更最多進行一次 Superpowers review") with the 3-run-ceiling
      wording.
- [ ] 2.2 Update the Queue execution contract's "既有的每個 code batch 一次
      Superpowers review" phrase to reflect the 3-run ceiling instead of
      unlimited batches.
- [ ] 2.3 Add a new Codex paragraph next to the existing Copilot paragraph in
...
- [ ] 2.4 Confirm the CodeRabbit pre-authorization section is left unchanged
      and does not reference Codex.

## 3. `plugins/jt-flow/README.md`

- [ ] 3.1 Mirror the 3-run local-review ceiling wording from task 2.1.
- [ ] 3.2 Mirror the new Codex paragraph from task 2.3 next to the existing
      Copilot description.

## 4. `CLAUDE.md` (repository root)

- [ ] 4.1 Mirror the 3-run local-review ceiling wording in the review
      ...
      review".
- [ ] 4.2 Mirror the new Codex paragraph next to the existing Copilot
      guidance ("Copilot permits at most one review per PR or change...").

## 5. Tests

- [ ] 5.1 In `scripts/jt-flow-review-policy.test.mjs`, replace the batch-based
...
- [ ] 5.2 Add assertions in the same file that `SKILL.md`, `README.md`, and
...
- [ ] 5.3 Run `npm test` and confirm all `scripts/*.test.mjs` files pass,
      including the updated `jt-flow-review-policy.test.mjs`.

## 6. Verification

- [ ] 6.1 Run `npm run validate` and confirm it passes (marketplace checks,
      Release Please version sync, markdown lint, and the Node test suite).
- [ ] 6.2 Run `openspec validate --strict` and confirm this change passes.
- [ ] 6.3 Manually re-read the four updated documents
