...
- 不放寬 release eligibility 的 feat／fix 判定，亦不加入人工 merge fallback；只修正其輸入為 mainline delivery units，而非 raw Compare 的所有可到達提交。
...
### Mainline release eligibility and merge mode

Build #105 證明 Compare 的 `commits` 是「從 tag 到 target 可到達的提交」，不是「main 上的交付單位」：#216 的 side branch 內 `test(ci)` 提交雖可到達，卻沒有作為 main delivery 落地；真正的 first-parent unit 是 GitHub default merge commit，且其 body 帶有已通過 PR title gate 的 `feat(release)` title。release eligibility 因此必須以不可變的 `DRONE_COMMIT` 作 Compare target，從該 SHA 沿 `parents[0]` 回走至已發布 tag 的 base SHA，只分類這條 mainline。路徑缺失、重複、無法到達 tag、或不合法 delivery subject 一律 fail closed。

此 target 的未來 PR merge mode 固定為 squash-only，並將 PR title 設為 squash commit title；auto-merge 也顯式以 `merge_method: "squash"` 與已驗證的 release candidate title 寫入。這符合 Release Please 對線性歷史的建議，並讓 push-side subject guard、release eligibility 與 changelog 都讀到同一個 Conventional Commit。為只回復既有 #216 所留下的 delivery，parser 例外接受精確 GitHub default merge subject `Merge pull request #<number> from <owner>/<branch>`，且只從空行後的第一個 non-empty body line 取得並重新驗證 Conventional Commit title；任何其他 merge body 形狀都不被接受。這不是人工 release fallback。
...
單元測試以 mock GitHub API 覆蓋 valid protected squash merge、無 candidate no-op、偽造作者／branch／body、額外檔案、版本漂移、額外 CHANGELOG version block、錯 base／head、未 clean required checks、缺少 strict／admin enforcement、候選等待時 main 已改變、不可 merge、GitHub 拒絕 stale merge 後 main 已改變 no-op 與 API error。release eligibility tests 必須覆蓋 mainline squash、#216 型的嚴格 GitHub default merge recovery、side-branch `test` 忽略、缺失／斷裂 first-parent path 與 `DRONE_COMMIT` target binding。Drone structural tests 驗證 pipeline trigger、dependency、concurrency、token boundary、固定 CLI 版本與指令；repo-standard policy tests 專門驗證 npm／MCP template 也要求 auto-merge 與 target-compatible merge mode。
...
- **Behavior：** 對於 first-parent mainline 含 unreleased `feat` 或 `fix` 的 delivery commit `C`，Release Please 建立 candidate 後，`release-pr-auto-merge` 只在 `validate(C)` 與 `release(C)` 成功時以 squash 合併；docs／chore-only mainline 範圍維持既有成功跳過行為。Compare side parents 不會改變資格。
...
3. 在 PR 中執行 repository 與 Spectra validation；設定並讀回 `main` 的 latest-base required checks、automation-enforced protection，以及 target-compatible squash-only merge policy；不得手動改 release-managed version。
