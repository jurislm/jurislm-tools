...
- 將 Spectra 的 `proposal → design → specs → tasks` 寫為唯一變更追蹤紀錄；
  不建立、不引用、也不依賴 GitHub Issue。

## Non-Goals

- 不修改全域 `CLAUDE.md` 的 review orchestration、`jt-flow` 審查邏輯或任何外部服務設定。
...
- Priority：normal。
- Hard dependencies：無。
- Acceptance dependencies：`CLAUDE.md` 與 `jt-flow-review-orchestration`
  的現行規則。
- External blockers：無。
...
影響本 repo 的流程規則、`repo-standards` plugin 文件與其 living OpenSpec
specification。
