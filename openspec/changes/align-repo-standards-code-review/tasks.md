...
- [x] 3.3 Remove GitHub Issue workflow surfaces — Explicit invocation authorizes OpenSpec preparation：移除 `jt-flow-one` 對 GitHub Issue external context 的流程，並以 authorization delta 與 policy test 驗證 Spectra artifacts 是唯一 current record。
...
- [x] 3.7 Active changes use OpenSpec delivery records before queueing：讓 queue eligibility 只讀 Spectra Delivery Relations，不讀或建立 GitHub Issue inventory，並以 queue delta 與 queue execution test 驗證。
