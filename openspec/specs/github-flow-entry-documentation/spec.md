...
Define the repository branch-model claims that entry documents and active
OpenSpec context must keep aligned with verified GitHub state.
...
`openspec validate <change-name> --strict`, and SHALL verify the remote
branch-state claim through a separate GitHub readback.
